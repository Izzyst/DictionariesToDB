    ...
    public class WordMapp : ClassMap<Word>
    {
        public WordMapp()
        {
            ...
            HasMany(x => x.Defs) //HasMany is creating a one-to-many relationship with Defs (one Word to many Defs)
                ...
                .Cascade
                .All().Not.LazyLoad(); //Cascade.All on HasManyToMany tells NHibernate to cascade events down to the entities in the collection (so when you save the Store, all the Products are saved too).
